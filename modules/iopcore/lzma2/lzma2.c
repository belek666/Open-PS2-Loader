...
{
    printf("LZMA2 iop uncompress module\n");
    if (argc > 1) {
        lzma2_pkt_t pkt;
        memcpy(&pkt, argv[1], sizeof(lzma2_pkt_t));
        int ret = lzma2_uncompress(pkt.in, pkt.in_size, pkt.out, pkt.out_size);
        printf("LZMA2 IOP: lzma2_uncompress: %d\n", ret);
    }
